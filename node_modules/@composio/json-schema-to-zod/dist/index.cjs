...
var z16 = __toESM(require("zod/v3"), 1);
...
function parseAllOf(jsonSchema, refs) {
  ...
  return import_v3.z.intersection(parseAllOf({ allOf: left }, refs), parseAllOf({ allOf: right }, refs));
}
...
var parseArray = (jsonSchema, refs) => {
  ...
  if (Array.isArray(jsonSchema.items)) {
    return import_v33.z.tuple(
      jsonSchema.items.map(
        (v, i) => parseSchema(v, { ...refs, path: [...refs.path, "items", i] })
      )
    );
  }
  let zodSchema = !jsonSchema.items ? import_v33.z.array(import_v33.z.any()) : import_v33.z.array(parseSchema(jsonSchema.items, { ...refs, path: [...refs.path, "items"] }));
...
var parseEnum = (jsonSchema) => {
  if (jsonSchema.enum.length === 0) {
    return import_v37.z.never();
  }
  if (jsonSchema.enum.length === 1) {
    return import_v37.z.literal(jsonSchema.enum[0]);
  }
  if (jsonSchema.enum.every((x) => typeof x === "string")) {
    return import_v37.z.enum(jsonSchema.enum);
  }
  return import_v37.z.union(
    jsonSchema.enum.map((x) => import_v37.z.literal(x))
  );
};
...
var parseIfThenElse = (jsonSchema, refs) => {
  ...
  return import_v38.z.union([$then, $else]).superRefine((value, ctx) => {
...
var parseNot = (jsonSchema, refs) => {
  return import_v310.z.any().refine(
...
var parseNullable = (jsonSchema, refs) => {
  ...
  const baseSchema = hasNullDefault ? omit(omit(jsonSchema, "nullable"), "default") : omit(jsonSchema, "nullable");
...
var parseNumber = (jsonSchema) => {
  let zodSchema = import_v312.z.number();
  let isInteger = false;
...
var parseOneOf = (jsonSchema, refs) => {
  ...
  return import_v313.z.any().superRefine((x, ctx) => {
  ...
  for (const key of propertyKeys) {
    ...
    if (!required && propJsonSchema && typeof propJsonSchema === "object" && "default" in propJsonSchema) {
      if (propJsonSchema.default === null) {
        const hasAnyOfWithNull = propJsonSchema.anyOf && Array.isArray(propJsonSchema.anyOf) && propJsonSchema.anyOf.some(
          (schema) => typeof schema === "object" && schema !== null && schema.type === "null"
        );
        const hasOneOfWithNull = propJsonSchema.oneOf && Array.isArray(propJsonSchema.oneOf) && propJsonSchema.oneOf.some(
          (schema) => typeof schema === "object" && schema !== null && schema.type === "null"
        );
...
function parseObject(objectSchema, refs) {
  ...
  if (propertiesSchema) {
    ...
    if (hasPatternProperties) {
      ...
      if (additionalProperties instanceof z14.ZodNever) {
        output = z14.object({}).strict();
      } else if (isAdditionalPropertiesTrue) {
        output = z14.object({}).passthrough();
      } else {
...
var parseString = (jsonSchema) => {
  let zodSchema = import_v314.z.string();
...
var addDefaults = (jsonSchema, zodSchema, refs) => {
